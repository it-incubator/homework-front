...
const DATE_KEY = 'hw9-date' // для LS
...
export const Homework12 = () => {
  ...
  const start = () => {
    // 📝 запустить часы (должно отображаться реальное время) - https://learn.javascript.ru/settimeout-setinterval#setinterval

    // 📝 сохранить ID таймера в state
    // 🧙‍♂️ setInterval возвращает ID таймера. По умолчанию TypeScript воспринимает setInterval как функцию из Node.js,
    // а на браузерного API и типизирует ID как NodeJS.Timer. Чтобы подсказать TypeScript, можно явно указать,
    // что мы работаем с браузерным API, и вызвать функцию как свойство window: `window.setInterval(() => {...})`

    // 📝 сохранить новую дату в LocalStorage
    // 🧙‍♂️ В LocalStorage данные хранятся в виде строки. Преобразовать дату в строку можно с помощью метода .toString()

    const timerId = window.setInterval(() => {
      const newDate = new Date()
      setDate(newDate)
      localStorage.setItem(DATE_KEY, newDate.toString())
    }, 1000)
